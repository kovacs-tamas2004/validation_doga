...
let isLetter = chr => {
    if(/[a-z]/.test(chr) || /[A-Z]/.test(chr) || /[öüóőúéáűíÖÜÓŐÚÁŰÉÍ]/.test(chr)){
        return true;
    }
    else{
        return false;
...
let isPasswordStrong = pass => {
    if(pass.length >= 12 && /\d/.test(pass) && /[a-z]/.test(pass) && /[A-Z]/.test(pass) && /[-_+#.&@$]/){
        return true;
    }
    else{
        return false;
